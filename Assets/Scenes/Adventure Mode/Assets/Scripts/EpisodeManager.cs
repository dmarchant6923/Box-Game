...
public class EpisodeManager : MonoBehaviour
{
    ...
    EnemyManager[] enemies = new EnemyManager[0];
    Vector2[] enemyPositions = new Vector2[0];
    bool[] enemiesKilled = new bool[0];
    ...
    void Start()    
    {
        ...
        enemiesKilled = new bool[enemies.Length];
        for (int i = 0; i < enemies.Length; i++)
        {
            enemyPositions[i] = enemies[i].GetComponent<Rigidbody2D>().position;
        }
    ...
    void Update()
    {
        ...
        for (int i = 0; i < enemies.Length; i++)
        {
            if (enemies[i].respawn)
            {
                enemiesKilled[i] = true;
            }
            else if ((enemies[i] == null || enemies[i].enemyWasKilled == true) && enemiesKilled[i] == false)
            {
                enemiesKilled[i] = true;
            }
        }
        ...
        {
            int killedEnemies = 0;
            foreach (bool enemyKilled in enemiesKilled)
            {
                if (enemyKilled)
                {
                    killedEnemies++;
    ...
    IEnumerator EpisodeFinished()
    {
        ...
        if (UIManager.rawTime <= timeToBeat)
        {
